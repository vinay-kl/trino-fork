        ...
        implements VendedCredentialsProvider
{
    ...
    public VendedCredentialsHandle getFreshCredentials(ConnectorSession session, VendedCredentialsHandle handle)
    {
        ...
        //
        // Limitation: queryCompleted() (which evicts entries by queryId) is only called on the coordinator
        // via ConnectorMetadata.cleanupQuery(). Worker JVMs never receive this callback, so their write
        // cache entries persist until the 1-hour TTL expires. This is benign because queryId is unique —
        // no future query will match a stale entry — but entries do occupy memory until evicted by TTL or
        // LRU (maximumSize). A ConnectorPageSinkProvider-level cleanup hook would fix this but does not
        // exist in the current SPI.
    ...
    // Called by ConnectorMetadata.cleanupQuery() on the coordinator only.
    // Worker JVMs never receive this callback — their write cache entries expire via TTL.
    @Override
