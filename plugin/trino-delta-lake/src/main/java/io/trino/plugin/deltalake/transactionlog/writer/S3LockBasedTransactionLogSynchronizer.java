        ...
        implements TransactionLogSynchronizer
{
    ...
    @Inject
    S3LockBasedTransactionLogSynchronizer(DeltaLakeFileSystemFactory fileSystemFactory, JsonCodec<LockFileContents> lockFileContentsJsonCodec)
    {
    ...
    {
        writeWithFileSystem(fileSystemFactory.create(session, tableLocation), session, clusterId, newLogEntryPath, entryContents);
    }

    @Override
    public void write(ConnectorSession session, String clusterId, Location newLogEntryPath, byte[] entryContents)
    {
        writeWithFileSystem(fileSystemFactory.create(session), session, clusterId, newLogEntryPath, entryContents);
    }

    private void writeWithFileSystem(TrinoFileSystem fileSystem, ConnectorSession session, String clusterId, Location newLogEntryPath, byte[] entryContents)
    {
